...
func (repository Memcached) GetByEmail(email string) (users.User, error) {
	...
	if err != nil {
		if errors.Is(err, memcache.ErrCacheMiss) {
			return users.User{}, fmt.Errorf("email not found")
		}
...
func (repository Memcached) Update(user users.User) error {
	...
	emailKey := emailKey(user.Email)
	if err := repository.client.Set(&memcache.Item{Key: emailKey, Value: data}); err != nil {
		return fmt.Errorf("error updating email in memcached: %w", err)
	}
...
func (repository Memcached) Delete(id int64) error {
	...
	emailKey := emailKey(user.Email)
	if err := repository.client.Delete(emailKey); err != nil {
		if !errors.Is(err, memcache.ErrCacheMiss) {
			return fmt.Errorf("error deleting email from memcached: %w", err)
		}
